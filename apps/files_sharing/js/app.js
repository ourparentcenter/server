...
OCA.Sharing.App = {
	...
	_restoreShareAction() {
		...
		fileActions.registerAction({
			...
			actionHandler(fileName, context) {
				...
				$.post(generateOcsUrl('apps/files_sharing/api/v1/deletedshares/{shareId}', { shareId }))
	...
	_acceptShareAction() {
		...
		fileActions.registerAction({
			...
			actionHandler(fileName, context) {
				const shareId = context.$file.data('shareId')
				$.post(generateOcsUrl('apps/files_sharing/api/v1/shares/pending/{shareId}', { shareId }))
					.success(function(result) {
						context.fileList.remove(context.fileInfoModel.attributes.name)
					}).fail(function() {
		...
		fileActions.registerAction({
			...
			actionHandler(fileName, context) {
				const shareId = context.$file.data('shareId')
				$.ajax({
					url: generateOcsUrl('apps/files_sharing/api/v1/shares/{shareId}', { shareId }),
					type: 'DELETE',
				}).success(function(result) {
					context.fileList.remove(context.fileInfoModel.attributes.name)
				}).fail(function() {
