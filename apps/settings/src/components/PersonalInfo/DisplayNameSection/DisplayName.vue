...
<template>
	<div class="displayname-container">
		...
		<div class="displayname-actions-container">
...
export default {
	...
	methods: {
		onDisplayNameChange(e) {
			this.$emit('update:displayName', e.target.value)
		...
		async updatePrimaryDisplayName() {
			try {
				...
				if (this.displayName === '') {
					this.handleResponse('error', t('settings', 'Unable to delete full name'), e)
				} else {
					this.handleResponse('error', t('settings', 'Unable to update full name'), e)
				}
			}
		},

		isValid() {
			return this.displayName.trim() === this.displayName && this.displayName.trim() !== ''
		},
